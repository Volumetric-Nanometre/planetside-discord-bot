...
class Ps2PersonalEvents(Ps2EventClient,commands.Cog):
    ...
    @commands.command(name='ps2-personal-death-start')
    async def player_death_start(self,ctx,message):
        # NOTE: Depending on player activity, this script will likely exceed the
        # ~6 requests per minute and IP address limit for the default service ID.
        print('player_death func')
        if 'personal-death' not in Ps2EventClient.client:
            print("Creating client")
            await Ps2EventClient.start_event_client('personal-death')
        ...
        if int(char.id) in self.membersBeingTracked_id:
            ...
            await ctx.send("Player tracking started")
            self.membersBeingTracked_id.append(int(char.id))
            self.trackingdata.update({char.name(): {'deaths':int(0)}})
            ...
            async def player_death_generic(event):
                ...
                self.trackingdata.update({char.name(): {'deaths':int(total)}})

                print(f'{char.name()} has died')
                #await ctx.send(f'{char.name()} has died')
                #await ctx.send(f'{char.name()} death total is {total}')
    
    @commands.command(name='ps2-personal-death-stop')
    async def player_death_stop(self,ctx,message):
        # NOTE: Depending on player activity, this script will likely exceed the
        # ~6 requests per minute and IP address limit for the default service ID.
        print('player_death func')
        if 'personal-death' not in Ps2EventClient.client:
            print("No stats currently recording")
            return
        ...
        if int(char.id) in self.membersBeingTracked_id:
            ...
            print("Player removed from tracking id")            
            try:
                client.find_trigger('personal-death')
            except:
                pass
            ...
            print('Player not currently being tracked')
